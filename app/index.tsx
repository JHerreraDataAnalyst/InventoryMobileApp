...
const MenuButton = ({ title, subtitle, href }: MenuButtonProps) => (
  ...
  const loadMetrics = async () => {
    try {
      ...
      setMetrics({
        ...
        todaySales: todaySalesResponse.data.summary?.total_sales || 0,
        todayRevenue: todaySalesResponse.data.summary?.total_revenue || 0,
      });
    } catch (error) {
    ...
    <ScrollView style={styles.container}>
      ...
      <View style={styles.menuContainer}>
        ...
        <MenuButton
          ...
          href="/today-sales"
        />
        <TouchableOpacity style={styles.menuButton} onPress={loadMetrics}>
          <Text style={styles.menuTitle}>Actualizar</Text>
          <Text style={styles.menuSubtitle}>Recargar métricas</Text>
        </TouchableOpacity>
      </View>
    </ScrollView>
  );
...
const styles = StyleSheet.create({
  ...
  headerSubtitle: { fontSize: 16, color: '#ecf0f1', marginTop: 5 },
  metricsContainer: { 
    flexDirection: 'row', 
  ...
  metricValue: { fontSize: 20, fontWeight: 'bold', color: '#2c3e50' },
  metricLabel: { fontSize: 12, color: '#7f8c8d', marginTop: 5 },
  ...
  menuTitle: { fontSize: 18, fontWeight: 'bold', color: '#2c3e50' },
  menuSubtitle: { fontSize: 14, color: '#7f8c8d', marginTop: 5 },
});